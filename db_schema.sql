...
CREATE TABLE temas (
    ...
    FOREIGN KEY (tema_padre_id) REFERENCES temas(id)
);

-- Tabla de Relación Contenido-Tema
CREATE TABLE contenido_tema (
    contenido_id INTEGER,
    tema_id INTEGER,
    relevancia FLOAT,
    PRIMARY KEY (contenido_id, tema_id),
...
CREATE INDEX idx_temas_padre ON temas(tema_padre_id);
CREATE INDEX idx_contenido_tema_contenido ON contenido_tema(contenido_id);
CREATE INDEX idx_contenido_tema_tema ON contenido_tema(tema_id);
CREATE INDEX idx_metadatos_contenido ON metadatos(contenido_id);
